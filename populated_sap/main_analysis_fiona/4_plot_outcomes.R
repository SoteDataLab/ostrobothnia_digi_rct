...
# NOTE: minor changes from the SAP to plots (axis limits, axis breaks, etc.) 
...
# treatment time: 
treatment_start <- as.Date('2025-04-15') # NOTE: THE INFORMATION CAMPAIGN STARTED IN THE 1ST WEEK OF MAY.
...
ggplot(data$y1,
  ...
  geom_vline(xintercept = data$y1[, max(relative.time)], linetype = 'dashed') +
  ...
  scale_y_continuous(breaks = c(-0.5, seq(-0.5, 0.5, by=0.05), 0.5),
                     limits = c(-0.05, 0.10)) +
  ...
  scale_x_continuous(breaks = c(0, seq(50, 300, by=50), 365),
                     limits = c(0, data$y1[, max(relative.time)])) + 
...
data$y2$Outcome <- factor(data$y2$Outcome, 
                          levels = c('d.1', 'referrals', 'ed.in.person',
                                     'ed.other', 'hospital.visit'))
...
ggplot(data$y2,
  ...
  geom_hline(yintercept = 0, linetype = 'dashed') + 
  geom_vline(xintercept = 0, linetype = 'dashed') +
  geom_vline(xintercept = data$y2[, max(relative.time)], linetype = 'dashed') +
  labs(y = 'Difference in cumulative contacts per capita (T-C)',
       x = 'Days relative to treatment',
       linetype = 'Outcome: ',
       col = 'Outcome: ') +
  ...
  scale_linetype_manual(values = 1:5, labels = labels) +
  scale_x_continuous(breaks = c(0, seq(50, 300, by=50), 365),
                     limits = c(0,  data$y2[, max(relative.time)])) +
  theme(text = element_text(size=20),
        legend.position = 'bottom',
        legend.key.width = unit(4, 'cm'),
        panel.background = element_rect(fill = 'white', colour = 'white'),
...
labels <- c(d.1 = 'Digital clinic contacts (D.1)',
            occup.inperson = 'Occupat. HC: in-person (Y3.1)',
...
ggplot(data$y3,
  ...
  geom_hline(yintercept = 0, linetype = 'dashed') + 
  geom_vline(xintercept = 0, linetype = 'dashed') +
  geom_vline(xintercept = data$y3[, max(relative.time)], linetype = 'dashed') +
  labs(y = 'Difference in cumulative contacts per capita (T-C)',
       x = 'Days relative to treatment',
       linetype = 'Outcome: ',
       col = 'Outcome: ') +
  ...
  scale_linetype_manual(values = 1:5, labels = labels) +
  scale_x_continuous(breaks = c(0, seq(50, 300, by=50), 365),
                     limits = c(0, data$y3[, max(relative.time)])) +
  theme(text = element_text(size=20),
        legend.position = 'bottom',
        legend.key.width = unit(4, 'cm'),
        panel.background = element_rect(fill = 'white', colour = 'white'),
