...
# Annualize post-treatment outcomes (not D.2, however):                        # NOTE: THIS WAS REGISTERED IN THE PAP BUT NOT YET IMPLEMENTED IN THE SAP CODES.
...
vars <- paste0(vars, '_post')

dt[, (vars) := lapply(.SD, function(x) x * (12/9)), .SDcols = vars]          # NOTE: UPDATE THE MULTIPLIER ONCE WE GET MORE DATA!


# Estimation function
estimate <- function(data, Y = 'y1.1_post', D = 'd.1_post') {
  # INPUTS:
  ...
  gc() # NOTE: FOR SOME STRANGE REASON WE NEED TO ADD THIS HERE; OTHERWISE "*** recursive gc invocation" WHICH CAN PRODUCE FATAL ERROR
  ...
  tsls <- fixest::feols(
    Y ~ 0 | y1.1_pre + (occup.total_pre > 0) + ika + sukup + kunta31_12 + 
      income_percentile + kieli_k + distance.quartile + ek_kela_kansansairaus + 
      ek_kaisa_monisairas | D ~ treated, data=dt) # NOTE: typo/bug noticed in the SAP (D_1 instead of D), now fixed.
  ...
  p <- fs['treated', 'Pr(>|t|)']
  results.fs <- data.table(effect, se, p, type='fs') # NOTE: REMOVED QVALS 09/04/2026
  
  # Collect results (reduced form):
  effect <- rf['treated', 'Estimate']
  se <- rf['treated', 'Std. Error']
  p <- rf['treated', 'Pr(>|t|)']
  results.rf <- data.table(effect, se, p, type='rf') # NOTE: REMOVED QVALS 09/04/2026
  
  # Collect results (2SLS):
  effect <- tsls['fit_D', 'Estimate'] # NOTE: typo/bug noticed in the SAP (D_1 instead of D), now fixed.
  se <- tsls['fit_D', 'Std. Error'] # NOTE: typo/bug noticed in the SAP (D_1 instead of D), now fixed.
  p <- tsls['fit_D', 'Pr(>|t|)'] # NOTE: typo/bug noticed in the SAP (D_1 instead of D), now fixed.
  results.tsls <- data.table(effect, se, p, type='tsls') # NOTE: REMOVED QVALS 09/04/2026
...
pan_c <- unique(results[type == "tsls" & Y %in% c("y1.1_post", "y1.2_post") &
                          D=='d.1_post', # NOTE: typo/bug here in the SAP; we need to specify the first stage variable here.
...
specs <- list(
  ...
  list(D='d.1_post', Y='ed.other_post'),                                        # NOTE: Order (Y2.2 and Y2.3) fixed.
...
# NOTE: ADDED 09/04/26: calculate q-values
q_values_rf <- data.table(qval = sharpened_qvals(results[type == "rf", p]), 
                          Y = results[type == "rf", Y],  type = "rf")
q_values_tsls <- data.table(qval = sharpened_qvals(results[type == "tsls", p]), 
                            Y = results[type == "tsls", Y], type = "tsls")
qvals <- rbind(q_values_rf, q_values_tsls)
 
# merge these to results table
results <- merge.data.table(results, qvals, all.x = T)
