...
input_soteorg <- "/DATAINFRA/SOTEorg/cleaned/sote_2025_10_nimet_251009.csv" # NOTE: THIS IS UPDATED (VS. OUR PAP) SO THAT IT INCLUDES THE DIGITAL CLINIC
...
inputs <- inputs[(grepl("(avohilmo(.*)202[4-6]{1})|hta", inputs))]
...
inputs_kela <- list.files("~/DATAINFRA/KELA/cleaned", 
                          full.names = T)
inputs_kela <- inputs_kela[grepl('laakarinpalkkiot_202[4-6]{1}', inputs_kela)]

# Hospital visits
inputs_hilmo <- list.files(input_folder, full.names = T)
inputs_hilmo <- inputs_hilmo[grepl("hilmo_tupva202[4-6]{1}", inputs_hilmo)]

# Referrals
inputs_referral <- list.files(here(input_folder, "sidetables_avohilmo"), 
                              full.names = T)
inputs_referral <- inputs_referral[grepl("lahete_202[4-6]{1}", inputs_referral)]


# Have a look at all the input files
print(c(inputs_avosh, inputs_hta, inputs_hilmo, inputs_referral, inputs_kela))
...
# Define study window:
min_date <- as.Date('2024-04-15')
max_date <- as.Date('2026-01-14')
treatment <- as.Date('2025-04-15') # NOTE: THE INFORMATION CAMPAIGN STARTED IN THE 1ST WEEK OF MAY.
...
# Keep only ED departments (ed_centers --> ed_centers$tunniste_oid_s)
soteorg_ed <- soteorg[tunniste_oid_s %in% ed_centers$tunniste_oid_s & 
                        vuosi == 2025 & kk == 10]
...
private <- private[kaynti_pv >= min_date & kaynti_pv <= max_date]               # NOTE: BUG FIXED; kaynti_pv <= max_date (NOT <)
...
# Keep only rows that are within the study window
hilmo <- hilmo[tupva >= min_date & tupva <= max_date]                 # NOTE: BUG FIXED; tupva <= max_date (NOT <)
...
## fixed/added 13/03/2026
hilmo[urgent_in_person == 1 | urgent_other == 1, outpatient := 0]
gc()
...
# Keep only rows that are within the study window
refs <- refs[lahete_pvm_c >= min_date & lahete_pvm_c <= max_date]            # NOTE: BUG FIXED; lahete_pvm_c <= max_date (NOT <)
...
dt <- dt[date <= max_date, # the date on which the follow-up ends
