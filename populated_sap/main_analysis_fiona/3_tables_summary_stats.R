...
# Inputs:
input_study_pop <- here('data', 'study_population.csv')
...
phc <- fread(input_phc_study, na.strings = c(NA_character_,''), 
             select = c('shnro', 'd.1_post', 'd.2_post', 'y1.1_post', 
                        'y1.3_post', phc_vars))
...
phc <-fread(input_phc_total, na.strings = c(NA_character_,''), 
            select = c('shnro','y1.1_post', 'y1.3_post', phc_vars))
...
dt_2[, customer := fcase(d.2_post == 1, "digi", 
                         d.2_post == 0 & y1.3_post >= 1, "trad")]               # NOTE: the SAP codes restricted to in-person visits whereas the purpose
dt_2[, .N, by = customer]                                                       # was to restrict to all traditional contacts -- now fixed.
...
smd <- dt_2[, sapply(.SD, function(x){
  smd(x = x, g = customer, gref = 1, na.rm = T)$estimate                             # NOTE: FIXED gref = 1 (not gref = 2)
}), .SDcols = vars_table]

# Add it to summary table:
table[, smd := smd]

# Mean & sd in the total treated population:
mean_study_p <- dt[treated == 1, sapply(.SD, mean, na.rm = T), .SDcols = vars_table]  # NOTE: the SAP codes did not restrict the total population to the treatment
sd_study_p <- dt[treated == 1, sapply(.SD, sd, na.rm = T), .SDcols = vars_table]      # group as it should have -- now fixed.

# Add them to the summary table too:
table[, mean_study_p := mean_study_p]
table[, sd_study_p := sd_study_p]
...
ns <- lapply(list(digi_mean_sd = dt_2[customer == 'digi'],
                  ...
                  mean_study_p = dt[treated == 1]),                             # NOTE: the SAP codes did not restrict the total population to the treatment
             uniqueN, "shnro")                                                  # group as it should have -- now fixed.
table <- rbind(table, c(variable = "N", ns), fill = T)
